...
class EventDetector(nn.Module):
    def __init__(
        ...
        if pretrain:
            # Correct the keys in the state_dict
            corrected_state_dict = {}
            for k, v in state_dict_mobilenet.items():
                if k == "classifier.1.weight":
                    corrected_state_dict["classifier.weight"] = v
                elif k == "classifier.1.bias":
                    corrected_state_dict["classifier.bias"] = v
                else:
                    corrected_state_dict[k] = v

            net.load_state_dict(corrected_state_dict)
